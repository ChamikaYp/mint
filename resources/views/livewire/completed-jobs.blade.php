...
<script>
  ...
  function createImagePreview(url, viewMode) {
    ...
    // Create anchor tag that opens the image in a new tab
  ...
  function downloadImage(imageUrl) {
      ...
      fetch(imageUrl, {
          mode: 'cors', // Ensure CORS is enabled on your S3 bucket
      })
      .then(response => response.blob()) // Convert the response to a blob
      .then(blob => {
          const url = window.URL.createObjectURL(blob); // Create a temporary URL for the blob
          const filename = imageUrl.substring(imageUrl.lastIndexOf('/') + 1); // Extract the filename from the URL
